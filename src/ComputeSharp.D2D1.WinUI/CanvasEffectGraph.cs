...
public readonly ref struct CanvasEffectGraph
{
    ...
    /// <remarks>
    /// This method can be called from both <see cref="CanvasEffect.BuildEffectGraph"/> and <see cref="CanvasEffect.ConfigureEffectGraph"/>.
    /// It can be used for efficiently changing the output node of a graph, without having to rebuild it. For instance, this can be used for
    /// effects that have multiple paths that can be taken for their inputs, depending on the value of some property describing the effect behavior.
    /// </remarks>
