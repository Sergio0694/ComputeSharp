...
{
    /// <summary>
    /// The shared method table for all <see cref="ID3D12MemoryAllocator"/> instances.
    /// </summary>
    [FixedAddressValueType]
    private static readonly ID3D12MemoryAllocatorImplVftbl SharedVftbl;

    /// <summary>
    /// Initializes <see cref="SharedVftbl"/>.
    /// </summary>
    static ID3D12MemoryAllocatorImpl()
    {
        SharedVftbl.QueryInterface = &QueryInterface;
        SharedVftbl.AddRef = &AddRef;
        SharedVftbl.Release = &Release;
        SharedVftbl.AllocateResource = &AllocateResource;
    }

    /// <summary>
    /// Gets the shared method table pointer for all <see cref="ID3D12MemoryAllocator"/> instances.
    /// </summary>
