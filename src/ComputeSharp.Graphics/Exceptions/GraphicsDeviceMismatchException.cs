...
namespace ComputeSharp.Exceptions
{
    ...
    public sealed class GraphicsDeviceMismatchException : InvalidOperationException
    {
        ...
        /// </summary>
        internal GraphicsDeviceMismatchException(GraphicsDevice device, GraphicsResource resource)
            : base(GetExceptionMessage(device, resource))
        {
        }

        // Gets a proper exception message for a pair of device and resource
        [Pure]
        private static unsafe string GetExceptionMessage(GraphicsDevice device, GraphicsResource resource)
        {
