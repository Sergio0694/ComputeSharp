...
namespace ComputeSharp
{
    ...
    public sealed unsafe class ReadBackBuffer<T> : NativeObject, IMemoryOwner<T>
        where T : unmanaged
    {
        /// <summary>
        ...
        public ReadBackBuffer(GraphicsDevice device, int length, AllocationMode allocationMode)
        {
            ...
            this.d3D12Resource = device.D3D12Device->CreateCommittedResource(ResourceType.ReadBack, allocationMode, (ulong)this.sizeInBytes, device.IsCacheCoherentUMA);
        ...
        /// <summary>
        /// A <see cref="MemoryManager{T}"/> implementation wrapping an <see cref="ReadBackBuffer{T}"/> instance.
        /// </summary>
        private sealed class MemoryManager : MemoryManager<T>
        {
            /// <summary>
            /// The <see cref="ReadBackBuffer{T}"/> in use.
            /// </summary>
            private readonly ReadBackBuffer<T> buffer;

            /// <summary>
            /// Creates a new <see cref="MemoryManager"/> instance for a given buffer.
            /// </summary>
            /// <param name="buffer">The <see cref="ReadBackBuffer{T}"/> in use.</param>
            public MemoryManager(ReadBackBuffer<T> buffer)
            {
                this.buffer = buffer;
            }
