...
namespace ComputeSharp
{
    ...
    public sealed class ReadOnlyTexture3D<T, TPixel> : Texture3D<T>
        ...
        where TPixel : unmanaged
    {
        ...
        public T this[int x, int y, int z] => throw new InvalidExecutionContextException($"{nameof(ReadOnlyTexture3D<T, TPixel>)}<T>[int,int,int]");
        ...
        /// <remarks>This API can only be used from a compute shader, and will always throw if used anywhere else.</remarks>
        public T this[Int3 xyz] => throw new InvalidExecutionContextException($"{nameof(ReadOnlyTexture3D<T, TPixel>)}<T>[Int3]");
