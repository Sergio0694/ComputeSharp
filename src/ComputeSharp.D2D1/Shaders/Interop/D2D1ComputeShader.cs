...
/// <summary>
/// Provides methods to interop with D2D1 APIs and compile compute shaders or extract their constant buffer data.
/// </summary>
public static class D2D1ComputeShader
{
    /// <summary>
    /// Loads the bytecode from an input D2D1 compute shader.
    /// </summary>
    /// <typeparam name="T">The type of D2D1 compute shader to load the bytecode for.</typeparam>
    /// <returns>A <see cref="ReadOnlyMemory{T}"/> instance with the resulting shader bytecode.</returns>
    /// <remarks>
    /// This method will only compile the shader using <see cref="D2D1ShaderProfile.ComputeShader50"/> if no precompiled shader is available.
    /// <para>
    ...
    /// </para>
    /// <para>
    /// If the shader needs to be recompiled, the shader profile that will be used is <see cref="D2D1ShaderProfile.ComputeShader50"/>.
    /// </para>
    /// <para>
    /// If the input shader was precompiled, the returned <see cref="ReadOnlyMemory{T}"/> instance will wrap a pinned memory buffer (from the PE section).
    /// If the shader was compiled at runtime, the returned <see cref="ReadOnlyMemory{T}"/> instance will wrap a <see cref="byte"/> array with the bytecode.
    ...
    /// <summary>
    /// Loads or compiles the bytecode from an input D2D1 compute shader.
    /// </summary>
    /// <typeparam name="T">The type of D2D1 compute shader to load the bytecode for.</typeparam>
    ...
    /// <returns>The number of inputs for the D2D1 compute shader of type <typeparamref name="T"/>.</returns>
    ...
    /// <summary>
    /// Gets the available input descriptions for a D2D1 compute shader.
    /// </summary>
    /// <typeparam name="T">The type of D2D1 compute shader to get the input descriptions for.</typeparam>
    ...
    /// <summary>
    /// Gets the available resource texture descriptions for a D2D1 compute shader.
    /// </summary>
    /// <typeparam name="T">The type of D2D1 compute shader to get the resource texture descriptions for.</typeparam>
    ...
    /// <summary>
    /// Gets the buffer precision for the output buffer of a D2D1 compute shader.
    /// </summary>
    /// <typeparam name="T">The type of D2D1 compute shader to get the output buffer precision for.</typeparam>
    /// <returns>The output buffer precision for the D2D1 compute shader of type <typeparamref name="T"/>.</returns>
    ...
    /// <summary>
    /// Gets the channel depth for the output buffer of a D2D1 compute shader.
    /// </summary>
    /// <typeparam name="T">The type of D2D1 compute shader to get the output buffer channel depth for.</typeparam>
    /// <returns>The output buffer channel depth for the D2D1 compute shader of type <typeparamref name="T"/>.</returns>
    ...
    /// <summary>
    /// Gets the constant buffer from an input D2D1 compute shader.
    /// </summary>
    /// <typeparam name="T">The type of D2D1 compute shader to retrieve info for.</typeparam>
    /// <param name="shader">The input D2D1 compute shader to retrieve info for.</param>
    /// <returns>A <see cref="ReadOnlyMemory{T}"/> instance with the compute shader constant buffer.</returns>
    /// <remarks>
    /// This method will allocate a buffer every time it is invoked.
    /// For a zero-allocation alternative, use <see cref="SetConstantBufferForD2D1ComputeInfo"/>.
    /// </remarks>
    ...
    /// <summary>
    /// Gets the size of the constant buffer for a D2D1 compute shader of a specified type.
    /// </summary>
    /// <typeparam name="T">The type of D2D1 compute shader to retrieve info for.</typeparam>
    /// <returns>The size of the constant buffer for a D2D1 compute shader of type <typeparamref name="T"/>.</returns>
    ...
    /// <summary>
    /// Tries to get the constant buffer from an input D2D1 compute shader.
    /// </summary>
    /// <typeparam name="T">The type of D2D1 compute shader to retrieve info for.</typeparam>
    /// <param name="shader">The input D2D1 compute shader to retrieve info for.</param>
    /// <param name="span">The target <see cref="Span{T}"/> to write the constant buffer to.</param>
    ...
    /// <summary>
    /// Sets the constant buffer from an input D2D1 compute shader, by calling <c>ID2D1ComputeInfo::SetComputeShaderConstantBuffer</c>.
    /// </summary>
    /// <typeparam name="T">The type of D2D1 compute shader to set the constant buffer for.</typeparam>
    /// <param name="d2D1ComputeInfo">A pointer to the <c>ID2D1DrawInfo</c> instance to use.</param>
    /// <param name="shader">The input D2D1 compute shader to set the contant buffer for.</param>
    /// <exception cref="ArgumentNullException">Thrown if <paramref name="d2D1ComputeInfo"/> is <see langword="null"/>.</exception>
    /// <remarks>For more info, see <see href="https://learn.microsoft.com/windows/win32/api/d2d1effectauthor/nf-d2d1effectauthor-id2d1computeinfo-setcomputeshaderconstantbuffer"/>.</remarks>
    public static unsafe void SetConstantBufferForD2D1ComputeInfo<T>(void* d2D1ComputeInfo, in T shader)
        where T : unmanaged, ID2D1PixelShader
    {
        default(ArgumentNullException).ThrowIfNull(d2D1ComputeInfo);

        D2D1ComputeInfoDispatchDataLoader dataLoader = new((ID2D1ComputeInfo*)d2D1ComputeInfo);
